...
public class PusherArm {
    /**
     * Setpoint for the pusher arm tilt.  It is measured in "ticks" per revolution (typically 4096 per revolution of the motor).  While in distance control mode, the PID Controller will hold the motor as the desired number of ticks.
     */
     ...
     * 
     * The pusher arm uses an encoder to count ticks as it moves.  When the robot is initially powered on, the pusher arm tilt angle is in an unknown state until it reaches a limit switch at a known position.  If the motor is ever shutoff for an over current condition it is reset to an unknown position (since that should never happen).
     * 
