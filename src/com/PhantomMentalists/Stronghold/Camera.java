...
public class Camera
{
	...
	//Constants
	NIVision.Range HUE_RANGE = new NIVision.Range(80, 135);	//Default hue range for yellow tote
	NIVision.Range SAT_RANGE = new NIVision.Range(30, 175);	//Default saturation range for yellow tote
	NIVision.Range L_RANGE = new NIVision.Range(30, 150);	//Default value range for yellow tote
	double AREA_MINIMUM = 0.05; //Default Area minimum for particle as a percentage of total image area
	double UPPER_LONG_RATIO = 1.8; //Tote long side = 26.9 / Tote height = 12.1 = 2.22
	double LOWER_LONG_RATIO = 1.33;
	double SHORT_RATIO = 1.4; //Tote short side = 16.9 / Tote height = 12.1 = 1.4
//	double RATIO_DEAD = 0.1;
	double SCORE_MIN = 75.0;  //Minimum score to be considered a tote
	double VIEW_ANGLE = 64; //View angle for camera, set to Axis m1011 by default, 64 for m1013, 51.7 for 206, 52 for HD3000 square, 60 for HD3000 640x480
...
//			HUE_RANGE.minValue = (int)prefs.getInt("Hue min", HUE_RANGE.minValue);
//			HUE_RANGE.maxValue = (int)prefs.getInt("Hue max", HUE_RANGE.maxValue);
//			SAT_RANGE.minValue = (int)prefs.getInt("Sat min", SAT_RANGE.minValue);
//			SAT_RANGE.maxValue = (int)prefs.getInt("Sat max", SAT_RANGE.maxValue);
//			L_RANGE.minValue = (int)prefs.getInt("Val min", L_RANGE.minValue);
//			L_RANGE.maxValue = (int)prefs.getInt("Val max", L_RANGE.maxValue);
	...
	//		System.out.println(L_RANGE.maxValue);
			...
			NIVision.imaqColorThreshold(binaryFrame, frame, 255, NIVision.ColorMode.HSL, HUE_RANGE, SAT_RANGE, L_RANGE);
...
//					NIVision.imaq
					...
					par.Area = (par.BoundingRectRight-par.BoundingRectLeft)*(par.BoundingRectBottom-par.BoundingRectTop);
					...
					double height = par.BoundingRectBottom-par.BoundingRectTop;
...
//					accepted += 1;
//					double ratio = (largest.BoundingRectRight-largest.BoundingRectLeft)/(largest.BoundingRectBottom-largest.BoundingRectTop);
...
//							   " / Bottom Right   "+largest.BoundingRectRight+":"+largest.BoundingRectBottom + " / Area: "+largest.Area + " / Ratio: "+ratio);
//				}
				particles.sort(null);
...
//			boolean isTote = scores.Aspect > SCORE_MIN && scores.Area > SCORE_MIN;
		...
		{
			num++;
			double ratio = (par.BoundingRectRight-par.BoundingRectLeft)/(par.BoundingRectBottom-par.BoundingRectTop);
			avglarg += par.Area;
//			System.out.println("\nTarget,"+num+
//					   "\nTop Left,"+par.BoundingRectLeft+","+par.BoundingRectTop+
//					   "\nTop Right,"+par.BoundingRectRight+","+par.BoundingRectTop+
